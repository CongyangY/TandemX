...
| Quantification | The 108-run public-command development ablation completed: empirical controls improved all genome means but regressed at 1×; oracle error input was best overall but is not blind; controls+oracle were redundant. The post-hoc depth-gated candidate improved every development seed | Commit/CI and exact-output replay the FASTA speed fix; freeze untouched genomes for the candidate; real control specificity, calibrated sampling intervals, catalogue uncertainty and ploidy remain |
...
| Software quality | Commit c8b3a8b passed 502 Python and 16 Rust release tests plus format/Clippy, compile and artifact/link audits locally; hosted runs 34084539666/34084539713 passed. The current evidence/speed checkpoint passes 505 Python tests and all local Rust/artifact gates. The cascade remains non-default after two promotion gates failed | Commit/CI and replay the current FASTA speed fix; production defaults, clean install/release verification, workflow portability and repeated isolated timing remain |
| GitHub/reuse | `c8b3a8b` is on main and the working branch; the cascade failure, public quantify implementation, Figure 8 and retrospective source audit are published. `docs/current_status.md` is the explicit new-window restart record | Publish the quantify development evidence and speed checkpoint, then complete release portability and archive deposition |
