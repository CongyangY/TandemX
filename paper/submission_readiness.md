...
| Software quality | Commit 62892a6 passed hosted Ubuntu/macOS Python, Rust and wheel checks before held-out execution; the evidence archiver and Figure 7 source pass all 469 Python tests locally | Verify the evidence checkpoint in hosted CI; production defaults, clean install/release verification, workflow portability and complete command validation |
| GitHub/reuse | 62892a6 is on main and the working branch; the failed and passed held-out chains have compact hash-checked archives and editable six-panel figures in the working tree | Publish this evidence checkpoint, then complete a clean release, portability checks and archive deposition |
