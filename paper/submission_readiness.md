...
| Software quality | Commit a73398d passes 472 local Python tests, Rust formatting and Clippy; hosted runs 34048998182/34049010825 passed Ubuntu/macOS Python, executable Rust and wheel jobs. The packed-trace/batched-period replay preserves six/seven real discovery products exactly while reducing one-replay runtime and RSS. Restoration commit e760471 rejected a later metric tradeoff and passed runs 34050266939/34050278620 | Production defaults, clean install/release verification, workflow portability, complete command validation and repeated isolated timing |
| GitHub/reuse | `e760471` is on main and the working branch; the failed and passed held-out chains and accepted exact-output discovery optimization are published with compact hash-checked evidence. Classifier figures are editable six-panel SVG/PDF outputs | Complete a clean release, portability checks and archive deposition |
