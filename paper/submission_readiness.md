...
| Software quality | Commit f47fc1a passed 510 Python and 16 Rust release tests plus format/Clippy, compile and artifact/link audits locally; hosted runs 34087474642/34087489635 passed. The current validation freeze passes 514 Python tests, and the replay has 112 matching product hashes. The cascade remains non-default after two promotion gates failed | Commit/CI the validation freeze; production defaults, clean install/release verification, workflow portability and repeated isolated timing remain |
| GitHub/reuse | `f47fc1a` is on main and the working branch; the complete quantify development archive, Figure 9, FASTA shortcut and compact exact-output replay are published. `docs/current_status.md` is the explicit new-window restart record | Publish the validation freeze, then complete release portability and archive deposition |
