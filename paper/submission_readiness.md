...
| Assembly interpretation | Bounded bridging passed fresh 5501–5503 localization gates. On separate 5601–5603 classifier-development seeds, pooled blending improved sensitivity 59.88%→71.47%, FPR 1.03%→0.82% and precision 98.87%→99.24%, but failed leave-one-seed stability | Run the frozen seed-robust refinement before touching 5701–5703; biological under-representation and donor-matched validation remain unresolved |
...
| Software quality | Current source passes 453 Python tests; unchanged Rust source passed 15 tests, Clippy and formatting; 4bb3f5c passed hosted Ubuntu/macOS CI | Hosted CI for the seed-robust selector; production defaults, clean install/release verification, workflow portability and complete command validation |
| GitHub/reuse | 4bb3f5c pushed to main and working branch; raw multi-k checkpointing, classifier pre-registration, held-out localizer validation, YSD56 full QC and figure evidence are published; both hosted CI runs passed | Seed-robust selector needs the next verified commit/push; clean release, portability and archive deposition remain pending |
