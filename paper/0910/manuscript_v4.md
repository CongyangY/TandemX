...
tools differ in input, repeat representation and output granularity. The intended
contribution of TandemX is a family-resolved assembly-auditing workflow that links
read-first discovery and abundance estimation to assembly representation, with an
auditable record of supporting and conflicting evidence. It does not claim that
any individual discovery, occupancy or under-representation measurement is novel.
...
SRF reconstructs satellite units or higher-order repeats and estimates mapped-base
abundance [13]. Merqury and KAT assess assembly completeness and composition
through read–assembly k-mer comparisons [16,17]. TandemTools evaluates assembled
extra-long tandem repeats using long reads [18], while AniAnn’s annotates tandem
arrays in assembly sequences [19]. TandemX connects operational repeat-family
catalogues to abundance, assembly representation and explicit evidence states
...
representation in newer assemblies, showing that the read--assembly comparison
is concordant with improvements in repeat-rich sequence in these matched systems.
...
A targeted Ey15-2 recovery experiment produced no increase in repeat
representation; the complete negative result is reported in Supplementary Results.
