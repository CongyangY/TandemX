...
from benchmarks.challenge.trash_adapters import ARRAY_FILES, UNIT_FILES, csv_records, iter_regions, iter_units, iter_unit_extents
...
def evaluate(genome: Path, run: Path, outdir: Path) -> dict:
    ...
    try:
        ...
        rows = []
        coordinates = ('window_grid', 'r_extraction') if tool == 'trash' else ('one_based', 'unit_extent')
        for policy in coordinates:
            for period in ('native_peak', 'consensus_length'):
                all_regions = bounded_records(iter_unit_extents(run/'native', period) if policy == 'unit_extent'
                                              else iter_regions(run/'native', tool, policy, period))
        ...
        catalogues = []
        for secondary in ((False, True) if tool == 'trash' else (False,)):
            sequences, counts = region_catalogue(run/'native', tool, secondary)
            metrics, details = score_threshold_recovery(sequences, founders, .9)
            policy = 'primary_and_secondary' if secondary else 'primary'
            write_table(outdir/f'catalogue_{policy}_recovery.tsv', details,
                        ['truth_id', 'recovered', 'assigned_sequence_index', 'threshold', 'criterion'])
            catalogues.append(dict(consensus_policy=policy, **counts, **metrics,
                warning='native_sequence_length_scope30_1000;independent_of_period_accuracy;alternative_endpoint_not_selected_ranking'))
        write_table(outdir/'catalogue_metrics.tsv', catalogues, list(catalogues[0]))
        units = []
        ...
        receipt.update(complete=True, region_metrics=rows, catalogue_metrics=catalogues, unit_coverage_metrics=units,
