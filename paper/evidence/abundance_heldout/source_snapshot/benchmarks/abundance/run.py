...
from benchmarks.abundance.simulate import GenomeSpec, build_genome, sample_reads, write_genome
...
def run(config_path: Path, outdir: Path, split: str) -> None:
    ...
    GenomeSpec(seed=seeds[0], periods=tuple(config['periods']), copies=tuple(config['copies']), flank_bp=config['flank_bp']).validate()
    ...
    provenance.update(benchmark_source_sha256=benchmark_hashes, platform=platform.platform(), python=sys.version,
                      ...
                      scope="known-catalogue conditional quantification/localization; not discovery or empirical HiFi validation",
    ...
    for seed in config["seeds"][split]:
        spec = GenomeSpec(seed=seed, periods=tuple(config["periods"]), copies=tuple(config["copies"]), flank_bp=config["flank_bp"])
        genome_dir = outdir / "genomes" / f"s{seed}"
        ...
        for variant in manifest["variants"]:
            folder = outdir / "runs" / f"s{seed}" / variant["name"] / "locate"
            ...
            if ok:
                for row in score_localization(folder/"output"/"arrays.bed", retained, variant["genome_bp"]):
                    locate_scores.append(dict(seed=seed, assembly_fraction=variant["fraction"], **row))
        ...
        for coverage in config["coverages"]:
            for error in config["substitution_rates"]:
                ...
                reads_dir = outdir / "reads" / f"s{seed}" / name
                ...
                folder = outdir / "runs" / f"s{seed}" / name / "quantify"
                ...
                if not ok:
                    continue
                context = dict(seed=seed, coverage=coverage, substitution_rate=error, read_length=config["read_length"])
                ...
                for variant, retained, arrays, success in located:
                    if not success:
                        continue
                    comp = outdir / "runs" / f"s{seed}" / name / variant["name"] / "compare"
    ...
    for filename, rows in (("copy_number_metrics.tsv", cn_scores), ("localization_metrics.tsv", locate_scores),
                           ...
                           ("copy_number_summary.tsv", aggregate(cn_scores, ["coverage", "substitution_rate"], "copy")),
                           ("comparison_summary.tsv", aggregate(compare_scores, ["coverage", "substitution_rate", "assembly_fraction"], "comparison"))):
        if rows:
