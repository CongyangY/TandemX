...
def cgroup_cpu(before: Path, after: Path) -> dict:
    """Aggregate cgroup-v2 CPU counters, including all forked workers."""
    counters = []
    for path in (before, after):
        ...
        for line in path.read_text().splitlines():
            ...
            if len(fields) != 2 or fields[0] in values or not fields[1].isdigit():
                raise ValueError('Invalid cgroup CPU counters')
            values[fields[0]] = int(fields[1])
        if not {'usage_usec', 'user_usec', 'system_usec'} <= values.keys():
            raise ValueError('Missing cgroup CPU totals')
        counters.append(values)
    start, end = counters
    if start.keys() != end.keys() or any(end[k] < start[k] for k in start):
        raise ValueError('Cgroup CPU counters changed schema or decreased')
    delta = {k: end[k]-start[k] for k in start}
    return dict(delta_counters=delta, usage_seconds=delta['usage_usec']/1e6,
                user_seconds=delta['user_usec']/1e6, system_seconds=delta['system_usec']/1e6,
                scope='all_container_tasks_between_measurement_snapshots_including_small_wrapper_overhead')
...
def run(fasta: Path, outdir: Path, tool: str, image: str, timeout: float = 1800) -> dict:
    ...
    try:
        ...
        try:
            state = json.loads(checked(['docker', 'inspect', '--type', 'container', name]))[0]['State']
        except (RuntimeError, subprocess.TimeoutExpired) as exc:
            receipt['container_state'] = None
            receipt['startup_error'] = str(exc)
            raise RuntimeError('Container creation/startup failed or timed out; no inference success') from exc
        if state['Running']:
        ...
        if measured.exists():
            try:
                ...
                receipt['resource_error'] = str(exc)
        ...
        receipt['cgroup_cpu'] = None
        try:
            receipt['cgroup_cpu'] = cgroup_cpu(native/'cgroup_cpu_before.txt', native/'cgroup_cpu_after.txt')
        except (ValueError, OSError) as exc:
            receipt['resource_error'] = receipt.get('resource_error', '')+';cgroup_cpu: '+str(exc)
        ...
        try:
            cleanup = subprocess.run(['docker', 'rm', '-f', name], capture_output=True, text=True, check=False, timeout=60)
            receipt['cleanup'] = dict(exit_code=cleanup.returncode, stdout=cleanup.stdout, stderr=cleanup.stderr)
        except subprocess.TimeoutExpired:
            receipt['cleanup'] = dict(exit_code=None, error='cleanup_timeout;check_only_recorded_container_name')
