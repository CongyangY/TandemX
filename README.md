...
`tandemx discover --collapse-redundant-families` is optional and off by default. When enabled, it writes `collapsed_families.tsv`, `collapsed_monomers.fa`, and `family_collapse.tsv`, but only collapses pairs classified as `likely_redundant`. Pairs labelled `possible_higher_order_or_partial` are retained and should be reviewed. Every discovery/import also writes `family_hierarchy.tsv`, a directional candidate graph that separates near-integer period multiples from unresolved related/partial pairs while preserving all alternatives. These edges are heuristic evidence and are not validated higher-order repeats.
...
The [optional TRASH/TRASH2 comparator container](benchmarks/containers/trash/README.md)
