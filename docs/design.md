...
The default repeat catalog consumed by downstream modules is the output of
`tandemx discover`. A provenance-labelled catalogue from a supported
`tandemx import` is an explicit alternative detector route. Simulator truth
files are not part of the analysis workflow.
...
Inputs: reads, genome size, k-mer size and the repeat catalog generated by
`tandemx discover` or a supported provenance-labelled import (`families.tsv`
and `monomers.fa`).
...
Inputs: assembly FASTA, the repeat catalog generated by `tandemx discover` or a
supported provenance-labelled import, window size, step size and match
thresholds.
...
Inputs: the repeat catalog generated by `tandemx discover` or a supported
provenance-labelled import, `copy_number.tsv` and optional localization
evidence.
