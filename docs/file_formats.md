...
When direct `quantify --checkpoint-every N` is selected, the output directory
temporarily contains `quantify_scan.checkpoint.json`. It is an internal resume
file, not a scientific result. Version 1 stores a SHA-256-verified JSON body:
input path and full-file SHA-256 identities (read files, monomers, optional
controls), scientific settings and selected-target digest, processed read/base
counts, maximum read length, diagnostic target counts, exact read-prefix digest,
and quality-survival accumulators. It is atomically replaced and removed after
successful `copy_number.tsv` publication. A malformed file, mismatched identity,
incomplete read prefix, or coexisting public result fails closed. Fresh and
resumed `copy_number.tsv` use the same columns below.

Input catalog: `monomers.fa` generated by `tandemx discover` or a supported
provenance-labelled `tandemx import`.

Optional `--single-copy-kmers` input is a headered TSV with required columns
`kmer` and `expected_copy_number`. Every k-mer must contain A/C/G/T, have the
same length as `--k`, and be unique after reverse-complement canonicalization.
...
| median_kmer_depth | float | counts | Median diagnostic k-mer depth after multiplicity and configured k-mer-survival correction |
| haploid_depth | float | X | Haploid depth from explicit input, empirical single-copy controls, or total read bases/genome size, as named by `normalization_method` |
...
| single_copy_control_mean_depth | float/NA | X | Mean survival-corrected control depth divided by declared expected copy number, including zero-observation controls; used for empirical normalization |
| single_copy_control_median_depth | float/NA | X | Median survival-corrected control depth divided by declared expected copy number |
| single_copy_control_depth_mad | float/NA | X | Median absolute deviation of normalized control depths |
| single_copy_control_zero_fraction | float/NA | fraction | Fraction of supplied controls with zero observed count |
| quality_window_count | integer | windows | Number of valid k-mer windows whose survival used FASTQ quality values |
| confidence | string | NA | Confidence label |
| warning | string | NA | Semicolon-separated limits, including catalog-only diagnostic specificity, user-supplied control provenance and independent-error/Phred assumptions |

Older TandemX `copy_number.tsv` files do not contain these calibration-audit
fields. Readers continue to accept that earlier schema; new runs always emit them.
...
## TideHunter import outputs

Produced by: `tandemx import tidehunter`

The importer accepts the 11-field whitespace-delimited output produced by
TideHunter `-f 2` plus the original FASTA/FASTQ read files. It validates native
read lengths and intervals against a temporary disk-backed source-read index.
TideHunter's 1-based inclusive start/end fields become 0-based half-open
`read_start`/`read_end` values in `candidate_reads.tsv`. FASTA headers with
semicolon metadata retain their complete TideHunter-facing identifier as
`native_read_id`; `read_id` is TandemX's normalized identifier.
...
native TandemX detections.

`tidehunter_import.tsv` maps each normalized candidate to all native fields:
`candidate_id`, normalized `read_id`, `native_read_id`,
`tidehunter_repeat_id`, `native_copy_number`, `native_read_length`,
`native_start`, `native_end`, `native_consensus_length`,
`native_average_match_percent`, `native_full_length_copy_number`,
`native_subunit_starts`, `normalized_start`, `normalized_end`, `status`, and
`warning`.

`import_summary.json` records the native input path/SHA-256, source-read paths
and byte sizes, a semantic SHA-256 over ordered read IDs and sequences,
read/base/candidate/family counts, all clustering parameters, output hashes and
the external-detector warning. The semantic read hash intentionally excludes
quality strings because TideHunter receives sequence rather than quality
evidence. Empty native output is retained as a completed zero-candidate result
when the supplied read files validate.
...
| --- | --- |
| `genomes/*/truth_copy_number.tsv`, `assembly_*.truth.tsv` | `chrom`, `family_id`, 1-based `array_index`; `start/end` are 0-based half-open planted coordinates (equal for absent arrays); `period` in bp; integer `copies`; `repeat_bp=end-start`. A family may have multiple non-overlapping rows; copy and repeat totals are summed by family. |
...
| `copy_number_metrics.tsv` | Context `seed`, requested biological `unit_substitution_rate`, `array_fragments`, `fragment_gap_bp`, read `coverage`, `substitution_rate`, `read_length`; `family_id`, family-summed `truth_copies`, `estimate`; `signed_relative_error=(estimate-truth)/truth`, its `absolute_relative_error`; `interval_low/high`, Boolean `interval_contains_truth`, `(high-low)/truth` as `interval_relative_width`; `sampling_oracle_copy_estimate` from sampled planted bases/period/actual depth, `oracle_relative_error`, `(estimate-oracle)/truth` as `estimator_minus_sampling_oracle`; explicit `interval_kind` |
| `localization_metrics.tsv` | `seed`, scenario fields above, requested `assembly_fraction`, `family_id`; `true_assembly_bp`, union `predicted_assembly_bp`, `overlap_bp`; overlap/true as `base_recall`, overlap/prediction as `base_precision`, merged predicted interval count `fragments`, and nonempty planted interval count `truth_fragments` |
| `comparison_metrics.tsv` | Read/scenario context above plus requested `assembly_fraction`, `family_id`; actual family-summed integer-copy `truth_assembly_read_ratio`, Boolean `truth_underrepresented`, native `predicted_assembly_read_ratio` and `native_status`, Boolean `predicted_underrepresented`, `outcome` TP/FN/FP/TN |
