...
SIGKILL if needed. It also kills any descendant that survives after the group
leader exits. The pipeline records exit status 130, marks the step
...
initially returned 5 passed in 11.30 seconds. An additional descendant test
passed with the other process-cleanup tests (4 passed in 11.90 seconds). The checks cover normal stdout and
stderr forwarding, a real SIGINT delivered to a wrapper with a live child,
absence of that child after the wrapper exits, termination of a descendant
that ignores SIGTERM after its group leader exits, and the interruption receipt.
