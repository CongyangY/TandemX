...
On the first check the manifest validator returned `valid: true`, three structurally valid small files and one `invalid` K30076 record; 9 focused pytest cases passed. The schema version 2 gate then added explicit file content/path leakage and task-role/pairing checks; its 13 focused pytest cases passed and the same real input inventory remained valid with all three readable files `source-unresolved`. Neither result establishes stable T7 write reliability, matched biological truth or benchmark performance. Before any large acquisition or compute, inspect the connection and perform a separately authorized bounded write/readback gate in a new T7 directory, then revalidate every intended input file and freeze the formal protocol.
