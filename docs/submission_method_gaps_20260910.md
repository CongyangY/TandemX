...
* The guarded SRF development workflow has observed `ok`, `no_catalogue`,
  `no_eligible_kmers`, and failure fates.  Those prior runs remain development
  evidence; the separate 72-cell planted-read study is the completed formal
  common-endpoint comparison.
...
| --- | --- | --- | --- |
| TandemX is superior to SRF or ordinary competitive mapping on historical real reads | No completed matched historical comparison exists.  The frozen Ey15 SRF k151 attempt timed out during native assembly and yielded unavailable family estimates; the other five planned historical cells were not run. | Historical evidence supports TandemX-specific family prioritization and bounded orthogonal interpretation. | A complete, predeclared matched historical comparison would be required for this claim.  The completed planted-read study is not a historical real-read comparison. |
...
| --- | --- | --- |
| Formal comparator figure/table | The 72-cell planted-read comparison is complete and appears in Supplementary Table S2 and Supplementary Figure S6. | Preserve both SRF k settings, mapping `N/A` discovery, correspondence ambiguity/unassigned mass, per-family abundance errors and resource scope. Do not generalize this controlled comparison to historical real reads. |
...
5. Real historical cross-tool agreement or superiority remains unavailable.
   The frozen attempt ended with one technical-failure cell and five
   `not_run_prior_failure` cells; these and `N/A` are informative evidence
   states, not missing values to impute.
...
mapping on the real historical inputs.  The completed formal planted-read
study closes only its controlled-comparator gap; it does not erase
