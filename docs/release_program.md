...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Current source passes all 467 Python tests; unchanged Rust source passed 15 tests, fmt and clippy; commit 300e48d and both hosted held-out-guard CI runs passed on Ubuntu and macOS; the v3 freeze guard, raw-pair contract and depth-only selector pass locally, with hosted CI pending for this checkpoint |
...
| Localization/comparison | Known array coordinates; engineered collapse levels; false collapse calls and boundary errors | Bounded anchor bridging retained 0.973858 mean full-assembly recall and 0.999631 positive precision on fresh 5701–5703, but the double-frozen alpha-0.5/threshold-0.5 classifier failed held-out FPR and precision gates despite a 0.087791 sensitivity gain. All 16 added false positives were at nominal 1×; biological truth remains absent |
...
| Innovation | Audited prior art; method ablations; held-out evaluation; AI retained only with reproducible benefit | The transparent multi-k/depth rule failed domain transfer; bounded localization recovered; pooled classifier selection failed seed stability; and the seed-robust refinement then failed independent FPR/precision gates. The post-failure low-depth fallback is now frozen with immutable development hashes and untouched seeds 5801–5803, but has not yet been validated; biological validation remains pending |
...
| Paper | Abstract, background, results, methods, discussion; multi-panel figures; tables, supplement, source data and references | Evidence-backed draft plus six six-panel main figures and source tables exist; Figure 6 retains the adverse independent classifier result. Scientific completion, final bibliography/metadata and biological validation remain pending |
