...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Current source passes all 459 Python tests; unchanged Rust source passed 15 tests, fmt and clippy; commit 300e48d and both hosted held-out-guard CI runs passed on Ubuntu and macOS; the new archive/figure tests pass locally |
...
| Innovation | Audited prior art; method ablations; held-out evaluation; AI retained only with reproducible benefit | The transparent multi-k/depth rule failed domain transfer; bounded localization recovered; pooled classifier selection failed seed stability; and the seed-robust refinement then failed independent FPR/precision gates. Low-depth fallback is a post-hoc development hypothesis requiring a new frozen validation; biological validation remains pending |
