...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Current source passes all 453 Python tests; unchanged Rust source passed 15 tests, fmt and clippy; both 4bb3f5c hosted CI runs passed on Ubuntu and macOS; the seed-robust selector awaits its hosted checkpoint |
...
| Localization/comparison | Known array coordinates; engineered collapse levels; false collapse calls and boundary errors | Bounded anchor bridging passed fresh 5501–5503 localization gates, but the old classifier traded sensitivity for FPR/precision. On independent classifier-development seeds 5601–5603, pooled blending improved all three aggregate metrics but failed leave-one-seed stability; a seed-robust post-v1 selector is frozen and 5701–5703 remain untouched; biological truth remains absent |
...
| Innovation | Audited prior art; method ablations; held-out evaluation; AI retained only with reproducible benefit | The transparent multi-k/depth rule failed domain transfer; bounded localization recovered, and a transparent blend then failed seed-stability despite aggregate gains. Seed-robust minimax selection is frozen as development refinement; independent and biological validation remain pending |
...
| Paper | Abstract, background, results, methods, discussion; multi-panel figures; tables, supplement, source data and references | Evidence-backed draft plus five six-panel figures and source tables exist; scientific completion, final bibliography/metadata and biological validation remain pending |
