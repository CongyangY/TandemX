...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Commit c8b3a8b passed 502 local Python tests and hosted runs 34084539666/34084539713. The current quantify archive, Figure 9 and FASTA speed checkpoint passes 505 Python tests, 16 Rust release tests, Rust formatting/Clippy, compileall and artifact/link audits locally; commit, hosted CI and an exact-output replay remain required |
| Challenge benchmark | Independent seeds; indels, unit divergence, mixed families, multiple arrays, short arrays, hard negatives; strict normalization | Once-only seeds 3101–3103 are complete and archived: TandemX minimum positive array recall/precision 0.985714, zero negative-read calls and RSS ratio 0.455130 versus TideHunter. Promotion failed because nine TRF control runs timed out and TandemX/TideHunter wall-time ratio was 2.457943 (>2.0). Do not rerun or tune on these seeds |
| Quantification | Known copy number and coverage; finite-read and error bias; catalog ambiguity; calibration/interval coverage | The frozen public-command development ablation completed 108/108 runs. Empirical controls reduced aggregate MARE 0.401898→0.365728 and improved all genome means but regressed at 1×; oracle error input reached 0.359640 but is not blind. Controls+oracle were estimate-identical and slower. A post-hoc depth-gated candidate requires new frozen genomes; real controls and a calibrated interval remain pending |
...
| Paper | Abstract, background, results, methods, discussion; multi-panel figures; tables, supplement, source data and references | Evidence-backed draft plus nine six-panel main figures and source tables exist; Figures 8–9 retain the failed cascade promotion and quantification trade-offs. Scientific completion, final bibliography/metadata and biological validation remain pending |
