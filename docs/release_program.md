...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Current source passes all 431 Python tests; unchanged Rust source passed 15 tests, fmt and clippy; both 1231743 hosted CI runs passed, with the next evidence checkpoint CI pending |
...
| Real data | Public accession, source checksums, matched sample/assembly, bounded pilot then scaling; known-family recovery | Ten complete libraries across eight species and 262.731 Gb passed full-file QC; nine sampling ladders are complete and YSD56 is active; donor-matched truth remains pending |
