...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Frozen commit 9eda196 passed 514 local Python tests and hosted runs 34088623796/34088640260. Current post-validation source passes 520 Python tests, Rust format/Clippy, wheel build and isolated full toy workflow; local Rust tests compile but need hosted execution because conda lacks dynamic libpython. A same-machine replay retained byte-identical 5,940-row metrics and all 108 copy-number files while reducing full-matrix time 610.507→349.217 s; this remains an engineering check rather than publication timing |
