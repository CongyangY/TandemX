...
| Comparators | Expanded task-matched methods in comparator_matrix.md | TRF, TideHunter, ULTRA, SRF and TRASH1/2 executed at task-specific stages. A pinned TideCluster 1.21.2 image passed dependency QA, planted-assembly smoke, and nested 10/100-Mb MorexV3 reference-window execution. The latter is descriptive and reached about 7.4 GiB clustering RSS at both sizes; whole-genome/accuracy comparisons, unitFinder and other applicable assembly/genotyping tests remain pending |
...
| Paper | Abstract, background, results, methods, discussion; multi-panel figures; tables, supplement, source data and references | Evidence-backed draft plus eleven six-panel main figures and source tables exist; Figures 8–11 retain the failed cascade promotion, development trade-offs, passed frozen quantify validation and passed guarded-cascade validation. Scientific completion, final bibliography/metadata and biological validation remain pending |
