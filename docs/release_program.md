...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Commit c3798db passed 486 local Python tests and hosted runs 34080916140/34081012863. The current quantify/evidence checkpoint passes 502 Python tests, 16 Rust release tests, Rust formatting/Clippy, compileall and artifact/link audits locally; commit and hosted CI remain required before its frozen development run |
...
| Quantification | Known copy number and coverage; finite-read and error bias; catalog ambiguity; calibration/interval coverage | Three-genome/27-condition multi-k and joint-read experiments complete; 20× conditional interval coverage 94.58% with explicit missingness. Optional public-command Phred/error-survival and independently supplied control-depth normalization are implemented with a frozen four-method development ablation; execution, held-out selection and real calibration remain pending |
...
| Paper | Abstract, background, results, methods, discussion; multi-panel figures; tables, supplement, source data and references | Evidence-backed draft plus eight six-panel main figures and source tables exist; Figure 8 retains the failed cascade promotion rather than hiding it. Scientific completion, final bibliography/metadata and biological validation remain pending |
