...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Current source passes all 456 Python tests; unchanged Rust source passed 15 tests, fmt and clippy; both ebfaaf5 hosted CI runs passed on Ubuntu and macOS; the held-out double-freeze guard awaits its hosted checkpoint |
...
| Localization/comparison | Known array coordinates; engineered collapse levels; false collapse calls and boundary errors | Bounded anchor bridging passed fresh 5501–5503 localization gates, but the old classifier traded sensitivity for FPR/precision. Seed-robust post-v1 development selected alpha 0.5/threshold 0.5 and improved all three metrics within each 5601–5603 seed; double-frozen 5701–5703 validation remains untouched; biological truth remains absent |
