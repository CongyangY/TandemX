...
| --- | --- | --- |
| Engineering baseline | Full pytest; Rust tests and release build; output validation | Current source passes all 437 Python tests; unchanged Rust source passed 15 tests, fmt and clippy; both c15dad7 hosted CI runs passed on Ubuntu and macOS; expanded evidence-archiver changes await the next hosted checkpoint |
...
| Localization/comparison | Known array coordinates; engineered collapse levels; false collapse calls and boundary errors | Fresh 5201–5203 validation improved TP/FN/FP/TN from 198/45/14/148 to 208/35/12/150 with a frozen rule; the predeclared 1–5% unit-divergence and one/three-segment baseline consumed 5401–5403 and completed 1,062 commands; frozen-rule evaluation and biological truth remain pending |
