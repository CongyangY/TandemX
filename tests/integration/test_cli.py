...
def test_subcommand_help() -> None:
    for command in (
        "discover", "quantify", "locate", "probe", "compare", "cohort",
        "visualize", "simulate", "import", "validate",
    ):
...
def test_downstream_catalog_help_points_to_discover_output() -> None:
    for command in ("quantify", "locate", "probe"):
        result = run_cli(command, "--help")
        assert result.returncode == 0
        assert "generated by `tandemx discover`" in result.stdout
        assert "`tandemx import`" in result.stdout
