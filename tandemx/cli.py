...
def run_import_tidehunter(args: argparse.Namespace) -> int:
    from tandemx.importers.tidehunter import import_tidehunter_catalog

    _require_existing_files([args.input, args.reads])
    args.backend = resolve_kmer_backend(args.backend, args.kmer_size)
    args.outdir.mkdir(parents=True, exist_ok=True)
    _write_run_config(
        args.outdir, "import tidehunter", args, status="tidehunter_import_running"
    )
    logger = _configure_log(args.outdir, "import_tidehunter")
    logger.info("command=tandemx import tidehunter")
    logger.info("native_input=%s", args.input)
    logger.info("source_read_files=%s", len(args.reads))
    summary = import_tidehunter_catalog(
        args.input,
        args.reads,
        args.outdir,
        min_support_reads=args.min_support_reads,
    ...
    _write_run_config(
        args.outdir, "import tidehunter", args, status="tidehunter_import_completed"
    )
    logger.info(
        "status=tidehunter_import_completed candidates=%s families=%s",
        summary["candidate_count"],
        summary["family_count"],
    )
    print(
        f"tandemx import tidehunter: imported {summary['candidate_count']} candidates "
        f"into {summary['family_count']} families at {args.outdir}"
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    quantify.add_argument("--catalogue", "--catalog", required=True, type=_path_value, help="Repeat catalog FASTA generated by `tandemx discover` or a supported `tandemx import`. The --catalog spelling is accepted.")
    ...
    locate.add_argument("--catalogue", "--catalog", required=True, type=_path_value, help="Repeat catalog FASTA generated by `tandemx discover` or a supported `tandemx import`. The --catalog spelling is accepted.")
    ...
    probe.add_argument("--catalogue", "--catalog", required=True, type=_path_value, help="Repeat catalog FASTA generated by `tandemx discover` or a supported `tandemx import`. The --catalog spelling is accepted.")
    ...
    import_command = subparsers.add_parser(
        "import",
        help="Import supported external detector evidence into TandemX schemas.",
    )
    import_formats = import_command.add_subparsers(dest="import_format", required=True)
    tidehunter = import_formats.add_parser(
        "tidehunter",
        help="Import strict TideHunter -f 2 output and build a TandemX catalogue.",
        description=(
            "Validate TideHunter -f 2 calls against the original reads, normalize "
            "coordinates, retain native-field provenance, and build a TandemX "
            "operational monomer catalogue for downstream commands."
        ),
    )
    tidehunter.add_argument(
        "--input",
        required=True,
        type=_path_value,
        help="Native TideHunter -f 2 output with 11 whitespace-delimited fields.",
    )
    tidehunter.add_argument(
        "--reads",
        required=True,
        nargs="+",
        type=_path_value,
        help="Original FASTA/FASTQ input file(s), used for exact ID/length/coordinate validation.",
    )
    tidehunter.add_argument(
        "--min-support-reads",
        type=int,
        default=2,
        help="Minimum distinct source reads supporting an imported family.",
    )
    tidehunter.add_argument(
        "--cluster-identity",
        type=float,
        default=0.95,
        help="Minimum circular global edit similarity to a fixed observed representative.",
    )
    tidehunter.add_argument(
        "--backend",
        choices=("auto", "python", "rust"),
        default="auto",
        help="Sequence-distance backend for imported-consensus clustering.",
    )
    tidehunter.add_argument(
        "--family-audit",
        choices=("full", "related"),
        default="related",
        help="Pairwise catalogue audit mode; related uses an exact necessary-condition gate.",
    )
    tidehunter.add_argument(
        "--kmer-size",
        type=int,
        default=11,
        help="K-mer size for the post-clustering family relationship audit.",
    )
    tidehunter.add_argument(
        "--outdir",
        required=True,
    ...
    visualize.add_argument("--catalogue", "--catalog", required=True, type=_path_value, help="Repeat family catalogue produced by discover or a supported import.")
